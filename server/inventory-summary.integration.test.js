...
test('inventory summary and domestic board use complete source models and enforce page access', async () => {
  const dataDir = mkdtempSync(path.join(os.tmpdir(), 'caiwugongju-inventory-summary-'));
  ...
  writeFileSync(path.join(dataDir, 'caiwugongju.sqlite'), Buffer.from(legacyDatabase.export()));
  ...
  try {
    await waitForServer(`http://127.0.0.1:${port}/caiwugongju/`, child, logs);
    ...
    const persistedDatabase = new SQL.Database(readFileSync(path.join(dataDir, 'caiwugongju.sqlite')));
