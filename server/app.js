...
async function ensureAdmin() {
  ...
  if (!password) {
    console.warn('ADMIN_INITIAL_PASSWORD is not configured; administrator initialization was skipped.');
    return;
  }
...
app.use('/caiwugongju', express.static(distDir));
app.use(express.static(distDir));
app.get(/^\/caiwugongju\/(?!api).*/, (req, res) => res.sendFile(path.join(distDir, 'index.html')));
...
app.listen(port, () => {
  console.log(`Caiwugongju server running at http://localhost:${port}`);
});
