...
const dbPath = path.join(dataDir, 'caiwugongju.sqlite');
...
function backupDatabaseOnStartup() {
  try {
    if (fs.existsSync(dbPath)) {
      fs.copyFileSync(dbPath, path.join(dataDir, 'caiwugongju.backup.sqlite'));
    }
  } catch {
...
function startPeriodicDatabaseBackups() {
  ...
  const timer = setInterval(() => {
    try {
      ...
      fs.copyFileSync(dbPath, path.join(backupDir, `caiwugongju-${dateStr}-${hourStr}.sqlite`));
